...
function searchVal() {
    ...
    window.location.href="afterSalesProjects?pageSize="+pageSize+"&search="+search;
}
...
$(document).ready(function() {
    $('#submit').bind('click', function() {
        var eventFun = function() {
            $.ajax({
                type : 'GET',
                url : 'afterSalesProject/uploadprocess',
                data : {},
                dataType : 'json',
                success : function(data) {
                    $("#proBar").attr("style", "width:" + (data.nownum / data.allnum) * 100 + '%');
