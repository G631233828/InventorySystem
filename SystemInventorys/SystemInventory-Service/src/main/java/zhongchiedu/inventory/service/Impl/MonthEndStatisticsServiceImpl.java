...
//	}
	...
	public void automaticStatistics(String date) {
	    ...
	    log.info("开始自动统计库存，总条数：{}", findAllStock.size());
	    ...
	    findAllStock.forEach(stock -> {
	        try {
	            log.info("记录库存设备{},数量{}", stock.getName(), stock.getInventory());
	            MonthEndStatistics ms = new MonthEndStatistics();
	            ms.setMonthEndStockNum(stock.getInventory());
	            ms.setDate(Common.isEmpty(date) ? Common.fromDateYMD() : date);
	            ms.setStockName(stock.getName());
	            ms.setStockModel(stock.getModel());

	            // 供应商/区域 加强非空
	            ms.setStockSuppier(stock.getSupplier() != null && stock.getSupplier().getName() != null
	                    ? stock.getSupplier().getName()
	                    : "未知供应商");
	            ms.setStockArea(stock.getArea() != null && stock.getArea().getName() != null
	                    ? stock.getArea().getName()
	                    : "未知区域");
	            ...
	            else if (stock.getPrice() != null && Common.isNotEmpty(stock.getPrice().trim()) && !"0".equals(stock.getPrice().trim())) {
	                try {
	                    priceStr = new BigDecimal(stock.getPrice().trim()).setScale(2, RoundingMode.HALF_UP).toPlainString();
	                } catch (NumberFormatException e) {
	                    log.warn("库存[{}({})]原有价格格式非法，使用默认值0.00", stock.getName(), stock.getId());
	                }
	            } else {
	                log.warn("库存[{}({})]无有效价格，使用默认值0.00", stock.getName(), stock.getId());
	            }

	            String inPriceStr = inPrice.toPlainString();
	            if (inPrice.compareTo(BigDecimal.ZERO) == 0) {
	                log.info("库存[{}({})]当月无入库记录，入库单价=0", stock.getName(), stock.getId());
	            }

	            ms.setPrice(priceStr);
	            ms.setInprice(inPriceStr);
	            ...
	            log.error("处理库存[{}({})]失败，已跳过", stock.getName(), stock.getId(), e);
	        }
	    });

	    // 最终结果
	    log.info("✅ 库存自动统计完成：总条数={}，成功={}，失败={}",
	            findAllStock.size(), successCount.get(), failCount.get());
	}
//	public void automaticStatistics(String date) {
//	    // 拿到所有库存的数据
//	    List<Stock> findAllStock = this.stockService.findAllStock();
